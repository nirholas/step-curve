...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(StepCurveHook).creationCode, abi.encode(IPoolManager(address(manager)), 2 * Q96, Q96 / 100, Q96 / 2, 1e18, 30, "Step Curve LP", "STEP-LP")
        );
        hook = new StepCurveHook{salt: salt}(IPoolManager(address(manager)), 2 * Q96, Q96 / 100, Q96 / 2, 1e18, 30, "Step Curve LP", "STEP-LP");
